...
| `docker compose` | **読む。** `compose.yaml` は `.env` の値を一切使っていないので、読むこと自体が意図した動作ではない |

手順 5 でシェルに読み込むときは **`X_` だけに絞る**（理由は本書「疎通を確認する」冒頭）。

#### 引用符の扱い

読み手が 3 つあり、**`$` を含む値は書き方によって読み手ごとに違う値になる**。

| 書き方 | `docker compose` | bash（`set -a; . ./.env`） | `bootRun` |
| --- | --- | --- | --- |
| `KEY=$2b$12$…` | 展開して**壊す**（警告） | 展開して**壊す** | そのまま |
| **`KEY='$2b$12$…'`** | **正しい** | **正しい** | **正しい** |
| `KEY="$2b$12$…"` | 展開して**壊す**（警告） | 展開して**壊す** | そのまま |
| `KEY=$$2b$$12$$…` | 正しい | **PID に展開して壊す** | `$$` が残って**壊す** |

**シングルクォートだけが 3 者すべてで正しい。** `$$` は `docker compose` の
エスケープであってシェルのものではなく、bash では `$$` がプロセス ID になる。

これに当たるのは `ADMIN_PASSWORD_HASH`。bcrypt ハッシュが `$2b$12$...` の形のため、
囲まないと `docker compose` を実行するたびに警告が出て、
**ハッシュの一部がコンソールに表示される**。
...
**対のクォートは 3 者とも剥がす。** `bootRun` 側でそれを行うのは
`backend/build.gradle.kts` の `unquoteEnvValue`。ここだけ剥がさなかった時期があり、
警告を消すために囲んだ結果、`bootRun` にだけ `'$2b$12$...'` が渡って
**管理ログインが通らなくなった**（Spring Security は `\A\$2(a|y|b)?\$` と
先頭から照合するため、クォート 1 文字で一致しなくなる）。

ただし `bootRun` は**クォートの中身を解釈しない**。bash と `docker compose` は
ダブルクォートの中で変数を展開するが、`bootRun` はしない。
**囲むならシングルクォート**にしておけば、この差は出ない。
