...
| ユーザー ID の解決（[x-integration.md](x-integration.md) 第 3.1 節） | **実装済み** | ただし 1 回 $0.010 かかるため、**手順 5.2 で手で叩いた結果を DB に入れる**運用にしている |
| `source_account` への投入 | **投入手段なし** | 手順 5.3 で SQL を直接実行する。管理 API には無い |
| 取り込みの停止スイッチ | **実装済み**（`X_INGESTION_ENABLED`） | 反映にデプロイが要る点は未解決（[security.md](security.md) 第 9 章） |
| 打ち切り（連続 10 回失敗）からの復帰 | **戻す手段なし** | 第 9 章で SQL を直接実行する。押すだけで戻せると原因が残ったまま再開するため、管理 API には置かない |
...
- 請求サイクルは**購入日起点**で切られる（例: `Sep 2 - Oct 2`）。
  ...
  **起点の日をアプリ側の設定 `x.billing-cycle-start-day` に入れる**
  （[api.md](api.md) 第 5.7 節）。ずれていると管理画面の合計が
  支出上限のリセットと噛み合わない
...
手順 5 でシェルに読み込むときは **`X_` だけに絞る**（理由は第 5 章冒頭）。
...
| 手段 | 効果 | 副作用 |
| --- | --- | --- |
| **`X_INGESTION_ENABLED=false`** | スケジューラのビーン自体が作られない。**通常はこれ** | **反映にデプロイが要る**（[security.md](security.md) 第 9 章） |
...
**自分で止めたのか、連続失敗で止まったのかを取り違えない。** 後者は第 9 章で戻す。
...
[x-integration.md](x-integration.md) 第 7 章）。自動では再開しない。
...
原因が残ったまま再開してしまう（[api.md](api.md) 第 5.7 節）。
...
| `error_summary` | 原因 | 対処 |
| --- | --- | --- |
| `HTTP 401` / `403` | Bearer Token の失効・権限変更 | 再発行して `fly secrets set X_BEARER_TOKEN=…`（第 4 章） |
...
`GET /2/usage/tweets` の `project_usage` を確認する（第 7 章）。
...
2. **停止スイッチの反映にデプロイが要る**（[security.md](security.md) 第 9 章）。
   `X_INGESTION_ENABLED` は実装済みだが、管理画面から止められるようにするかは未決
3. **使用量エンドポイントの自動監視**。現状は手動確認。
   `fetched_resource_count` と X 側の実績を突き合わせる仕組みは未検討
4. **説明のつかない実行が 1 件ある**（付録 C.4）。取得件数を 2 と記録しながら
   投稿を 1 件も取り込んでいない実行があった。原因は特定できていない
...
| 2026-09-03 | **手順 9 打ち切りからの復帰** | **完了。課金なし。** ローカル DB に `FAILED` を 10 件入れて `halted: true` を確認し、第 9.2 節の `UPDATE` で最新 1 件を `CANCELLED` にしたところ `halted: false` に戻った（`consecutiveFailureCount` は 10 → 0） |
...
ISR で 300 秒キャッシュしているため（[architecture.md](architecture.md) 第 5.2 節）。
**不具合ではない。**
...
| 誤ったパスワードでの認証 | `{"authenticated":false}` + **HTTP 200**（総当たり対策どおり。第 6.1 節） |
