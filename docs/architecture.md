...
**`docker compose` はリポジトリ直下の `.env` を読み、値の中の `$` を
変数展開として扱う。** このハッシュがそれに当たり、`docker compose` を
実行するたびに「変数が未設定」という警告が出て**ハッシュの一部が
コンソールに表示される**。`compose.yaml` は `.env` の値を使っていないため
実害はなく、`bootRun` が渡す値にも影響しない（そちらは展開しない）。
気になる場合はハッシュを `.env` ではなく別の手段で渡す。
