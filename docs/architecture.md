...
#### 実測 — 上の試算は本番で成立していない（2026-09-15）

| | 稼働率 | 月換算 |
| --- | --- | --- |
| 試算（30 分間隔） | 約 17% | 約 31 CU-hours |
| **実測** | **約 95%** | **約 171 CU-hours** |

請求期間（2026-09-03 開始）の 12.4 日で **70.34 / 100 CU-hours**。日あたり 5.70 は、
0.25 CU を常時起動した場合の 6.00 CU-hours/日にほぼ等しい。
**コンピュートがほぼ suspend していない。**

上の試算は「取り込みのたびに 5 分だけ起動し、残りは停止する」を前提にしている。
**その前提が成立していない。** 何が DB を起こし続けているかは未特定。
...
このままだと **2026-09-20 ごろに無料枠を使い切り、請求期間が変わる 10/3 まで
