...
2. 小文字化
3. **保持する文字以外**をすべて除去
4. 除去の結果が空になったら、**原文の小文字化**を返す（`event_key` は `NOT NULL`）
...
| --- | --- | --- |
| ひらがな | `\p{IsHiragana}` | |
| カタカナ | `\p{IsKatakana}` | `ヶ`（U+30F6）・`ヴ`（U+30F4） |
| 漢字 | `\p{IsHan}` | **`々`（U+3005）**。Unicode の Script が `Han` なので残る |
| 英数字 | `a-z0-9` | 小文字化と NFKC の後なので大文字・全角は現れない |
| 長音符 | `ー`（U+30FC） | Script が `Common` で `\p{IsKatakana}` に入らないため**明示的に足している** |

**これ以外はすべて落ちる。** 判断が割れやすいものを名指しで挙げる。

| 文字 | 扱い | 例 |
| --- | --- | --- |
| `・`（U+30FB 中黒） | **落とす** | `ア・イ` → `アイ` |
| `〆`（U+3006） | **落とす**（Script が `Common`。`々` と違う） | `〆切ライブ` → `切ライブ` |
| `〜`（U+301C 波ダッシュ） | **落とす** | `波〜線` → `波線` |
| 記号・括弧・句読点・空白 | **落とす** | `『A』「B」` → `ab` |
| 絵文字 | **落とす** | `祭🎤` → `祭` |

**長音符を落とすと照合が壊れる。** `ラーメン` が `ラメン` になり、
同じイベントの再告知が別のキーになる。**明示的に残す理由はこれ**であって、
記号を一律に落とすという原則の例外ではない。

###### NFKC は文字そのものを変える

除去より先に走るため、**互換文字は別の文字に展開されてから**判定される。

| 原文 | `event_key` |
| --- | --- |
| `Ⅳ` | `iv` |
| `①` | `1` |
| `㈱テスト` | `株テスト`（`(株)` に展開され、括弧が落ちる） |
| `０１２` / `Ａｂ` | `012` / `ab` |

**実装と食い違わせない。** この表は `EventKeyTest` が固定しており、
正規表現を書き直すとテストが落ちる。片方だけ変えると、
既存行のキーと新しい告知のキーが一致せず**照合が静かに壊れる**。
...
**出演情報から参照されていない行（`appearance.venue_id` が 1 件も指していない行）
だけ削除できる**（[api.md](api.md)「会場の一覧と編集」）。

会場名の書き換えで行が取り残される。`東京・神田SQUARE HALL / 東京・渋谷WOMB LIVE`
のような連結行を管理者が正しい会場名へ直すと、**元の連結行は誰からも参照されないまま
残る**。一覧のノイズになるので消せるようにしている。

**参照されている会場を消せるようにしない。** `appearance.venue_id` は外部キーなので
DB が拒むが、その前にアプリ側で数えて `409` を返す。消せてしまうと、公開ページから
地域（色）も地図リンクも落ちる。
...
解決は**1 日 1 回の定期実行**が行う（ADR-0022）。登録時に作られるのは
`venue` の行までで、`place_id` は `NULL` のまま。
**未解決の間も地図リンクは名前検索で機能する**ため、遅れて困るものは無い。
...
地域（＝カレンダーの色）が遅れて付くのは避けたい（ADR-0022）。
...
**マイグレーションが作るのはテーブルと列だけ。** 既存行への `venue_id` の充填は
**定期実行が通常運用と同じ経路で**行う（ADR-0022「既存データの初期投入」）。

`venue_key` の正規化と地域の判定はアプリ側のロジックであり、
...
2. 見つからなければ、同じ `appearance_date` と `event_key` を持ち
   ...
   時刻なしの行は、管理者の手動登録か、タイムテーブル未確定の告知の取り込みで作られる
3. **登録しようとしているのが時刻なしで、同じ `appearance_date` と `event_key` の行が
   既にあるなら何もしない。** 一意キーは開始時刻を含むため、時刻ありの行があっても
   時刻なしの行は制約に触れずに作れてしまい、**カレンダーに同じ公演が 2 行並ぶ**。
...
初回バックフィルの範囲外に情報解禁があった場合など、逆順になる経路がある。
