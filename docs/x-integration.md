...
`docs/x-post-sample/` の実サンプル 19 件から導出した。
...
実サンプル 19 件から確認できた型。**取りこぼしではなく、意図して対象外にしている。**
...
```
探索範囲: 先頭 〜 開演時刻の行の直前
        （⏰ で始まる行、または OPEN hh:mm を含む行の、先に現れたほう）
        どちらもなければ ▪️ を含む最初の行の直前
        それも無ければ本文全体
```

**`⏰` が落ちた告知がある。** 実サンプル 19.txt は `OPEN 19:00 / START 19:30` と
マーカーが付いていない。`⏰` だけを見ると探索範囲が `▪️` まで広がり、
**販売期間の日付が公演日の候補に混ざる**（19.txt では `8/21(金)` と `8/28(金)`）。
候補が 3 つになると出演枠を日付に対応付けられず、投稿ごと `UNPARSED` に落ちる。
`OPEN hh:mm` も境界として扱うことで、マーカーの有無によらず同じ範囲になる。
...
**主催者名が別の行にあると取りこぼす。** 実サンプル 19.txt は
`HATENA CREATION x PLAYPIT presents` が括弧の行の**前の行**にあり、
イベント名は `「新進火花」` になる。`lonlium pre.『LONELY KIDS』` のように
同じ行に付く場合しか拾えない。**行をまたいで連結すると住所行（5.txt）まで
巻き込むため、現状は取りこぼす側に倒している。**
...
本章のルールを `docs/x-post-sample/` の実サンプル 19 件に適用した結果。
...
探索範囲が `▪️` まで広がると販売期間の `8/21(金)` `8/28(金)` が公演日の候補になり、
...
**14〜16.txt の `🔗` 行は X の表示形**（`🔗 ticketdive.com/event/TcX3Cpew…`）で
採取したため、スキームが無くチケット URL が取れない。実 API の本文は
`https://t.co/...` を含む（本番で登録された行のチケットリンクが
`https://t.co/NGGpRDBOgq` であることから確認できる）。
**この 3 件はチケット URL の検証には使えない。**
...
公演日として採用した日付は、対応する 12 件すべてで告知の曜日と実際の曜日が一致した
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 19 件が、対応 12 件 / 対象外 7 件に正しく仕分けられること（第 5.2 節） |
...
**当日中に反映しないと価値がゼロになる**情報で、実サンプル 19 件中 2 件と頻度も低くない。
