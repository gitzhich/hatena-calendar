...
- **投稿は古い順に処理する。** API は新しい順に返すため、逆順に並べ替えてから処理する。
  ...
  空欄補完（本書「既存レコードへの補完」）が意図通りに働かない
- **`last_fetched_tweet_id` の更新は全件処理後**に、取得できた最大 ID で一度だけ行う。
  ページ上限で打ち切った場合も**同じ**。未取得の古い側は諦める（本書「ページング」）
- 途中で失敗した場合は `last_fetched_tweet_id` を更新しない。
  ...
  その歯止めが本書「エラーハンドリング」の連続失敗による打ち切り
...
- この比較を成立させるため `BIGINT` で保持している（[data-model.md](data-model.md)「source_account — 情報源アカウント」）
...
初回（`since_id` なし）の扱いは本書「初回バックフィル」。
...
`note_tweet` を見ないと本書「物販時刻」の抽出が成立しない。
...
取得位置（`last_fetched_tweet_id`）は本書「処理順序の原則」どおり、
...
（本書「初回バックフィル」、[requirements.md](requirements.md)「外部依存と制約」）。
...
（[runbook-x-api-setup.md](runbook-x-api-setup.md)「コンソールで紛らわしい点」）、暦月で集計すると
支出上限のリセット日とずれる。アプリ側の起点は設定値
`x.billing-cycle-start-day`（1〜31。`1` で暦月と一致）で、
境界は JST で切る（[api.md](api.md)「取り込み履歴」）。
...
- `last_fetched_tweet_id` の後退（本書「取得位置を後退させない」）
- `expansions` の不要な指定
- **取得位置を進めず、毎回同じ範囲を取り直し続ける実装**（本書「処理順序の原則」と混同しない。
  失敗時に 1 回取り直すのは想定内で、地雷なのは位置が進まないこと）
- テストコードから実 API を叩くこと（本書「テスト方針」）
...
公式アカウントは公演告知以外の投稿もするため、まず**扱う投稿型を絞る**（本書「抽出対象の判定」）。
...
1. 探索範囲（本書「日付」）に日付候補がある
2. 探索範囲に `📍` がある
3. イベント名が取れる（本書「イベント名」）

**1 つでも欠ければ `UNPARSED`。** 部分的にしか埋まっていない出演情報を作らない。
日付とイベント名は `NOT NULL` で保存すらできず（[data-model.md](data-model.md)「appearance — 出演情報」）、
会場や時刻だけが入った行は誤情報として公開される。

#### 経路 A：タイムテーブルが確定している

`XINXIN` を含む `🎤` 行がある（本書「XINXIN の出演時刻」）。出演時刻まで取る。

#### 経路 B：タイムテーブルが未確定

`XINXIN` を含む `🎤` 行が無い。**出演時刻を持たない出演情報を 1 件だけ作る。**
後続の「タイムテーブル解禁」が空欄補完で時刻を埋める（本書「既存レコードへの補完」）。
...
| # | 条件 | 理由 |
| --- | --- | --- |
| B-1 | **`📍` の行から公演日がちょうど 1 つ取れる** | この経路は `▪️` も `🎤` も無く探索範囲が本文末尾まで広がるため、本書「日付」のフォールバックが販売期間の日付を拾う（23.txt は候補が 8 個になる） |
| B-2 | **イベント名が括弧付きで取れる** | 連結（本書「イベント名」）を許すと、括弧も空行も無い投稿で本文後半を名前にする。`event_key` が後続の告知と一致せず、**消えない重複行**になる |
| B-3 | **チケット URL が取れる**（本書「チケット URL」） | 「これから行われる公演の告知にはチケット情報が付き、過ぎた公演のお礼投稿には付かない」という意味の違いを条件にする。`🔗` は保持する項目なので、捨てる値の書き方が抽出の成否を決める形にならない |

B-3 が要求するのは**マーカーの存在ではなく、スキーム付きの URL が取れること**である
（本書「チケット URL」）。実 API の本文はリンクを `https://t.co/…` に短縮して返すため
スキームは必ず付く。実サンプルは抽出対象の 21 件すべてが `🔗` から URL を取れる（本書「実サンプルでの検証結果」）。
...
抽出が想定外の値を作ったら登録しない。**この判定も投稿単位**（本書「1 投稿から複数の出演情報」）。

#### 対応する投稿型

| 型 | 経路 | 実サンプル | 内容 |
| --- | --- | --- | --- |
| タイムテーブル確定の公演告知 | A | 2〜6.txt, 24.txt, 25.txt | 日付・会場・イベント名・出演時刻が 1 投稿に揃う |
| 前日告知 | A | 12.txt | 構造は上と同一。先行する告知で作られた行があれば空欄補完に吸収される（本書「既存レコードへの補完」） |
| 同じ日に 2 公演 | A | 16.txt, 17.txt, 26.txt, 27.txt | 日付の行で 2 ブロックに割れる（本書「1 投稿から複数の出演情報」）。日付と `📍` は同じ行でも別行でもよい |
| 公演情報解禁（出演時刻が未確定） | B | 1.txt, 21.txt, 22.txt, 28.txt | 日付・会場・イベント名だけが確定している |
...
実サンプル 28 件から確認できた型。**取りこぼしではなく、意図して対象外にしている。**

| 型 | 実サンプル | 対象外になる理由 | 扱い |
| --- | --- | --- | --- |
| 出演時間の訂正 | 8.txt, 11.txt | 日付が「本日」としか書かれず、日付候補が取れない | 将来対応（本書「将来対応する投稿型」） |
...
**公演告知の形をした、公演でない投稿は通る。** 経路 B が見るのは構造だけで、
「これが XINXIN の出演か」を判定する手立ては無い。次の 4 つが揃えば登録される。
...
| 投稿 | 結果 |
| --- | --- |
| グッズ告知に会場と日付を足した形（`9/15(火)📍東京・XINXIN物販ブース` / `『XINXIN漢字ネームT-SHIRT』` / `🔗`） | **登録される** |
| メンバー個人の出演告知（`9/15(火)📍東京・渋谷DESEO` / `『ソロイベント』` / `🔗`） | **登録される** |
| 動画公開の告知（日付も `📍` も無い） | `UNPARSED` |

**これを条件で塞ごうとしない。** 告知の種別行・`🎫`・`【出演者】` はいずれも
**保持しない項目**で、そこに依存すると書き方の揺れが抽出の成否を決める
（本書「日付」で外したばかりの構造）。誤って登録された行は点検一覧（FR-24）に
新しい順で並び、管理者が削除する（FR-23）。承認フローを置かない以上、
事後修正で直すのが設計の前提である（[ADR-0003](adr/0003-no-approval-flow.md)）。

**メンバー個人の出演を XINXIN の出演として登録してしまう点は経路 A も同じ。**
`🎤19:50-20:15 甘リ(XINXIN)出演` のような行は `XINXIN` を含むため経路 A を通る。
経路 B で新しく増えた露出は「公演告知の形をした非公演の投稿」だけである。
...
`{M}/{D}({曜日})📍{都道府県}・{会場}` で日付と会場が同一行にあり（本書「実際の投稿構造」）、
**販売期間の日付が `📍` の行に載ることはない。**
...
**経路 B ではフォールバックを使わない**（本書「抽出対象の判定」B-1）。`🎤` が無い投稿は
...
**素通りする**。`📍` の行に限れば、本書「実際の投稿構造」の基本形
`{M}/{D}({曜日})📍{都道府県}・{会場}` そのものが条件になり、
探索範囲の広さに依存しなくなる。

**境界を開演時刻に置かない。** 開演時刻（`⏰OPEN / START`）は**保持しない項目**であり
（本書「実際の投稿構造」）、そこを探索範囲の境界にすると**捨てる値の書き方が抽出の成否を決める**。
...
探索範囲はイベント名（本書「イベント名」）と枠会場（本書「会場」）の探索上限に使う。
**境界に使うのは `▪️` と `🎤`、つまり保持する項目のマーカーだけ**にする。
`▪️` は本書「会場」が既に会場の境界として使っており、2 つあった境界が 1 つになる。

**1 投稿に複数イベントが入る場合、探索範囲はブロックごとに取る**（本書「1 投稿から複数の出演情報」パターン C）。
...
- 都道府県を含んだ形のまま `venue_name` に入れる（[data-model.md](data-model.md)「実際の告知投稿との対応」）
- **ヘッダ会場は探索範囲（本書「日付」）の最初の 📍 から取る**。
...
```
ヘッダ会場が羅列でない（単一会場）
    枠の📍 あり → ヘッダ会場 / 枠の📍   例: 千葉・草ぶえの丘(千葉 佐倉) / Orange Shelter
                                            （枠の📍はステージ名にすぎない）
    枠の📍 なし → ヘッダ会場            例: 東京・渋谷CLUB QUATTRO（2.txt）

ヘッダ会場が羅列（サーキット・フェスで会場が並ぶ）
    枠の📍 あり → {都道府県}・{枠の📍}  例: 東京・渋谷WWW X（24.txt）
                                            （枠の📍が実際の出演会場。羅列は捨てる）
    枠の📍 なし → 空欄                   （どこに出るか決まらない。下記）
```

  都道府県はヘッダ会場の `・` より前から取る。
  サーキット形式でヘッダ会場をそのまま使うと、
  実サンプル 6.txt では 12 会場の羅列に出演会場が埋もれてしまう
- **羅列なのに枠の `📍` が無いなら空欄にする。** 羅列を入れてしまうと、
  空欄補完が値のある列を上書きしないため（[ADR-0004](adr/0004-fill-blanks-only.md)）、
  **タイムテーブル解禁が来ても実際の会場に直らない**。経路 B と同じ理由である
- **セクションの判定には `▪️` を必ず含める。** 「タイムテーブル」という語は
  1 行目のタイトル（`🔸XINXIN千葉公演タイムテーブル解禁🔸`）にも現れるため、
  語だけで判定すると本文全体がタイムテーブル扱いになり、
  会場が 1 つも取れなくなる（本書「実サンプルでの検証結果」の検証で実際に踏んだ）
- **単一会場で枠の `📍` がない告知では、ヘッダ会場をそのまま使う**（実サンプル 2.txt）。
...
```
ヘッダ会場が羅列でない  → ヘッダ会場をそのまま入れる
                            例: 東京・品川インターシティホール（21.txt）

ヘッダ会場が羅列        → 空欄。タイムテーブル解禁が実際の会場を埋める
...
| 区切り | 実サンプル |
| --- | --- |
| 半角 `/` | 1.txt（7 会場）/ 6.txt（12 会場） |
| `&` | 22.txt / 25.txt / 28.txt |
| 全角 `／` | 24.txt |

**判定を経路ごとに分けて持たない。** 以前は経路 A が `/` だけを見ており、
`&` と全角 `／` で並ぶ告知をサーキットと判定できなかった。その結果
**羅列と枠の会場を連結した値が本番に入った**
（`愛知・NAGOYA CLUB QUATTRO & RAD HALL / NAGOYA CLUB QUATTRO`）。
片方だけ直す事故を防ぐため、判定は 1 か所に置く。
...
**誤った値を公開するより取りこぼす**（本書「1 投稿から複数の出演情報」と同じ判断）。

連結してよいのは「括弧が無く、他に手がかりが無い」ときだけ。

**連結は経路 A でしか使わない**（本書「抽出対象の判定」B-2）。経路 B は `▪️` も `🎤` も無いため
...
**空欄補完が働かず、消えない重複行が残る**。
経路 B が取りこぼすのは実サンプル 28 件のうち 20.txt 型の 1 件だけで、
...
**行全体が消える**（本書「実サンプルでの検証結果」の検証で実際に踏んだ）。行はそのまま保持する。
...
- **該当する 🎤 行が複数あれば、それぞれが別の出演情報になる。**
  ...
  同じ日・同じイベントに並ぶ。1 行にまとめず 2 行として登録する（本書「1 投稿から複数の出演情報」）
- **見つからなければ投稿ごと対象外**にする（本書「抽出対象の判定」の条件 1）。
...
`TIME` 型は `24:00` 以上を表現できない（[data-model.md](data-model.md)「タイムゾーンの扱い」）。
...
- **`📸` 行には `XINXIN` の語が入らない。** ラベルは「終演後物販」「並行物販(ブースA)」
  ...
  そこで**本書「XINXIN の出演時刻」で採用した 🎤 行より後**に現れる最初の 📸 行を物販時刻とする
...
- 見つからない場合は `NULL` のままにする。エラーにしない。
  基準となる 🎤 行は必ず存在する（本書「抽出対象の判定」の条件 1）
- 24 時以上の表記は本書「XINXIN の出演時刻」と同じ規則で変換する。ただし
  **日付の繰り上げ判定が出演時刻と一致しない場合は物販時刻を保存しない**
  （`🎤23:30-23:55` に対する `📸24:10-25:00` など。
  [data-model.md](data-model.md)「タイムゾーンの扱い」）
...
- **マーカーと値の間の空白を許す。** 当初の実サンプル 13 件はすべて
  ...
  `🔗 https://...` とスペースを挟んでいた（本書「実 API で見つかった表記ゆれ」）。
...
抽出しない。理由は [data-model.md](data-model.md)「実際の告知投稿との対応」。
...
- 枠の物販時刻は、その 🎤 行より後の最初の 📸 を取る（次の 🎤 行を越えない。本書「物販時刻」）
...
- 見出しの日付は**月日だけで対応付け、曜日は検証しない**。
  実サンプル 6.txt では見出しの曜日が誤記になっている（本書「日付」）
...
- **ブロックの開始は「日付を持つ行で、その行または次の行に `📍` があるもの」。**
  この行が 2 つ以上あればまとめ告知とみなし、その位置で本文を分割する。
  区切り線は装飾で、形も有無も投稿によって変わるため判定に使わない

```
26.txt  ☀️6/7(日)📍韓国・SETi LIVE HALL    ← 同じ行
27.txt  ☀️6/14(日)                         ← 行が分かれている
        📍東京・神田SQUARE HALL
```

- **`▪️` の行は境界にしない。** タイムテーブル節の見出しであって、新しい公演の
  始まりではない。除かないと実サンプル 6.txt の `▪️8/25(土)タイムテーブル` →
  `📍NAGOYA ReNY limited` が境界になり、**1 公演 2 枠の告知が 2 公演に割れる**
- **各ブロックを 1 投稿として扱う。** 探索範囲（本書「日付」）・会場（本書「会場」）・
  イベント名（本書「イベント名」）・チケット URL（本書「チケット URL」）は、
  すべて**そのブロックの中だけ**を見る
...
| 壊れ方 | 原因 |
| --- | --- |
| 会場が連結される | 後続ブロックの `📍` を前ブロックの「枠の `📍`」と誤認し、`{ヘッダ会場} / {枠の会場}` を作る（本書「会場」） |
| イベント名が使い回される | ヘッダから 1 回だけ取り、すべての枠に適用する（本書「イベント名」） |
| チケット URL が使い回される | 本文全体の最初の `🔗` をすべての枠に適用する（本書「チケット URL」） |
...
本書「抽出対象の判定」の判定と同じ単位であり、落ちる場所によらず扱いを変えない。

| 落ちる場所 | 例 |
| --- | --- |
| 抽出（本書「XINXIN の出演時刻」ほか） | 2 枠のうち 1 枠が日を跨ぐ（`🎤23:50-24:30`） |
...
本章のルールを `docs/x-post-sample/` の実サンプル 28 件に適用した結果。
**これをそのまま抽出テストの期待値にする**（本書「テスト方針」）。値は省略せず全文を載せる。
...
24.txt  日付      2026-07-07 (火)
        会場      東京・渋谷WWW X
        イベント名  『YORU-FES ~夜を駆けるサーキットSP~』
        出演時刻    14:35-15:00        物販時刻  15:15-16:15
        チケット    http://t-dv.com/yorufes_260707
        備考      ヘッダ会場は全角 ／ で 3 会場が並ぶ。羅列を捨て都道府県を前置する

25.txt  日付      2026-08-05 (水)
        会場      愛知・NAGOYA CLUB QUATTRO
        イベント名  『RAD iD LIVE-NO残業DAY SUMMER SP-』
        出演時刻    19:55-20:20        物販時刻  20:50-22:00
        チケット    https://t-dv.com/RADiDLIVE-NoOvertimeDay-SUMMERSP-0805
        備考      ヘッダ会場は & で 2 会場が並ぶ。22.txt と同じ区切りだが
                  タイムテーブルがあるので会場を確定できる

26.txt  1 投稿から 2 行。同じ日・同じ会場で 2 公演
        (1) 日付      2026-06-07 (日)
            会場      韓国・SETi LIVE HALL
            イベント名  「SETi FES vol.34」
            出演時刻    13:45-14:05        物販時刻  15:00-16:40
            チケット    http://tiget.net/events/490635
        (2) 日付      2026-06-07 (日)
            会場      韓国・SETi LIVE HALL
            イベント名  「ジエメイ ONEMAN LIVE in KOREA」
            出演時刻    19:20-19:40        物販時刻  21:00-22:40
            チケット    http://tiget.net/events/492193
        備考      枠の 📍 が無いのでヘッダ会場をそのまま使う。連結しない

27.txt  1 投稿から 2 行。同じ日に別会場で 2 公演
        (1) 日付      2026-06-14 (日)
            会場      東京・神田SQUARE HALL
            イベント名  「アイドル甲子園 in KANDA SQUARE HALL」supported by My-th -DAY2-
            出演時刻    12:35-13:00        物販時刻  13:20-14:40
            チケット    https://user.my-th.jp/login?redirect=/tickets/event/aikou_0614
        (2) 日付      2026-06-14 (日)
            会場      東京・渋谷WOMB LIVE
            イベント名  『 IDOL STORM 』
            出演時刻    17:05-17:25        物販時刻  17:35-18:35
            チケット    http://t-dv.com/IDOLSTORM_0614
        備考      日付行と 📍 行が別。販売期限が公演当日で日付候補が 2 つに見える

28.txt  日付      2026-10-18 (日)
        会場      （空欄。2 会場が & で並ぶ）
        イベント名  『 IDO-LIVE!! Circuit 』
        出演時刻    （なし）           物販時刻  （なし）
        チケット    http://eplus.jp/IDOLIVE
```

**21.txt は `🎫` が `📝` に化けている**（`📝前売エリア¥8,500`）。価格は保持しない項目で
判定にも使わないため、抽出結果は変わらない。**保持しない項目のマーカーに
依存しない設計**（本書「日付」）が効いている例になる。`🔗` は
`🔗 https://…` とスペース区切りだが、本書「実 API で見つかった表記ゆれ」の規則で拾える。
...
（本書「1 投稿から複数の出演情報」パターン C）。14.txt と 15.txt は同じ 2 公演を個別に告知したもので、
**16.txt の 2 行はそれぞれと一致する**。実際の告知はこの順で届き、
まとめ告知は既存行の空欄補完だけで済む（[data-model.md](data-model.md)「追加告知による空欄補完」）。
...
**公演日を `📍` の行から取るようにして、保持しない項目への依存を外した**（本書「日付」）。
イベント名が `「新進火花」` になり主催者名を落としているのは本書「イベント名」の既知の制限。

**17.txt もまとめ告知**だが、16.txt と違い**冒頭の前置きに `📍` が付く**。
ブロックの開始行より前を捨てないと、会場が `大阪・Music Club JANUSと` になる
（本書「1 投稿から複数の出演情報」パターン C）。17.txt はイベント名が括弧の外から始まる形
（`I to U $CREAMing!! 8周年記念 大阪主催「symmetric」`）でもあり、
本書「イベント名」の「行全体を取る」が効いている。
...
実害は無く、先に登録された行の値が空欄補完で保たれる（本書「既存レコードへの補完」）。
...
公演日として採用した日付は、対応する 21 件すべてで告知の曜日と実際の曜日が一致した
（年を 2026 と解釈した場合）。本書「日付」の曜日検証が機能することの裏付けになる。
...
（[data-model.md](data-model.md)「同一イベントの一意性と event_key」）ではこれらをすべて扱う。
...
照合と補完の規則は [data-model.md](data-model.md)「追加告知による空欄補完」、
照合キー `event_key` の生成規則は同 [data-model.md](data-model.md)「同一イベントの一意性と event_key」に定める。

取り込み側の前提:

- **投稿は古い順に処理する**（本書「処理順序の原則」）。「公演情報解禁 → タイムテーブル解禁」の
  順で反映しないと補完が働かない
- 1 投稿から複数の出演枠を抽出した場合、**枠ごとに照合する**（本書「1 投稿から複数の出演情報」）
  ただし**枠が 1 つでも成立しなければ投稿ごと `UNPARSED`** にする（同節）。
  照合は枠単位、成否の判定は投稿単位である
- `event_key` は抽出時に生成する。生成メソッドは手動登録・編集と共有する
  （[architecture.md](architecture.md)「設計上の原則」）
...
- 打ち切っても `last_fetched_tweet_id` を更新しない（本書「処理順序の原則」）
...
バックフィル時も 1 回の実行あたりのページ数上限（本書「ページング」）を適用する。
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 28 件が、対応 21 件 / 対象外 7 件に正しく仕分けられること（本書「抽出対象の判定」） |
...
| 照合キーの文字集合 | 保持する文字と落とす文字を `EventKeyTest` で固定する（[data-model.md](data-model.md)「同一イベントの一意性と event_key」） |
...
- 抽出対象の 21 件が期待どおり抽出できること（本書「実サンプルでの検証結果」）
- 対象外の 7 件が 1 件も抽出されないこと（本書「実サンプルでの検証結果」）
- 7.txt（次回予告つきのお礼）が `UNPARSED` になること。
  緩い条件なら通過するため、退行を検知する要になる
- 1.txt / 21.txt / 22.txt（経路 B）が**出演時刻なしで**抽出できること（本書「抽出対象の判定」）
- 1.txt / 22.txt の会場が**空欄**になること（本書「会場」）。羅列を入れると後から直らない
- 23.txt（複数日開催の出演日程解禁）が `UNPARSED` になること。
  探索範囲の日付で代用すると通過してしまう（本書「日付」B-1）
- 経路 B で `🔗` が無い / イベント名が括弧付きでない投稿が `UNPARSED` になること
- `🎤` 行があるのに `XINXIN` が入っていない投稿が `UNPARSED` になること（本書「抽出対象の判定」）
- 時刻なしの行がある公演に後続のタイムテーブルが届くと**1 行に統合される**こと。
  逆順（時刻ありが先）でも**行が増えない**こと（[data-model.md](data-model.md)「追加告知による空欄補完」）
- 8.txt / 11.txt（出演時間の訂正）が `UNPARSED` になること（本書「出演時間の訂正（実サンプル 8.txt / 11.txt）」）
- 12.txt（前日告知）が、先行する告知で作られた行と重複しないこと（本書「既存レコードへの補完」）
- **6.txt の 2 枠を入力順を変えて処理しても、行数と内容が一致すること。**
  昇順への固定が効いていることの検証（[data-model.md](data-model.md)「追加告知による空欄補完」）
- 6.txt から**同じ日・同じイベントで 2 行**の出演情報ができること（本書「1 投稿から複数の出演情報」パターン A）
...
- 連続して 10 回失敗したら取り込みが止まること（本書「エラーハンドリング」）
- 閾値を超えた `RUNNING` の実行記録があるとき、`FAILED` に倒して実行が続くこと（本書「多重起動の防止」）
- 閾値内の `RUNNING` があるとき、実行がスキップされること（本書「多重起動の防止」）
...
- ポーリング間隔は **30 分**とする。新規投稿がなければ X API の課金は発生しないが、
  ...
  間隔を短くすると CU-hours を消費する（[architecture.md](architecture.md)「運用コストの試算」）
- 実行基盤は Fly.io 上の Spring Boot `@Scheduled`（[architecture.md](architecture.md)「取り込みジョブ」）
- 監視すべき値:
...
初回バックフィル（本書「初回バックフィル」）は手動実行で定期ジョブから呼ばないため、
長時間実行と行き違わない。
...
**当日中に反映しないと価値がゼロになる**情報で、実サンプル 28 件中 2 件と頻度も低くない。
...
1. **抽出精度の目標水準**。[requirements.md](requirements.md)「未決定事項（要件レベル）」の
   同名の項目と同一。
   サンプルが 28 件では表記ゆれを網羅できていない。運用開始後に実データを蓄積して見直す
