...
実サンプル 20 件から確認できた型。**取りこぼしではなく、意図して対象外にしている。**
...
この経路は現状 `UNPARSED` に落ちている投稿にしか効かないため、
既存の抽出結果は 1 件も変わらない。
...
本章のルールを `docs/x-post-sample/` の実サンプル 20 件に適用した結果。
...
公演日として採用した日付は、対応する 13 件すべてで告知の曜日と実際の曜日が一致した
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 20 件が、対応 13 件 / 対象外 7 件に正しく仕分けられること（第 5.2 節） |
...
**当日中に反映しないと価値がゼロになる**情報で、実サンプル 20 件中 2 件と頻度も低くない。
