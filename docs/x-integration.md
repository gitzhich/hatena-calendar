...
スキームは必ず付く。実サンプルは抽出対象の 16 件すべてが `🔗` から URL を取れる（本書「実サンプルでの検証結果」）。
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 23 件が、対応 16 件 / 対象外 7 件に正しく仕分けられること（本書「抽出対象の判定」） |
...
- 抽出対象の 16 件が期待どおり抽出できること（本書「実サンプルでの検証結果」）
