...
`docs/x-post-sample/` の実サンプル 16 件から導出した。
...
実サンプル 16 件から確認できた型。**取りこぼしではなく、意図して対象外にしている。**
...
本章のルールを `docs/x-post-sample/` の実サンプル 16 件に適用した結果。
...
公演日として採用した日付は、対応する 9 件すべてで告知の曜日と実際の曜日が一致した
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 16 件が、対応 9 件 / 対象外 7 件に正しく仕分けられること（第 5.2 節） |
...
**当日中に反映しないと価値がゼロになる**情報で、実サンプル 16 件中 2 件と頻度も低くない。
