...
スキームは必ず付く。実サンプルは抽出対象の 20 件すべてが `🔗` から URL を取れる（本書「実サンプルでの検証結果」）。
...
| 同じ日に 2 公演 | A | 26.txt | 日付と `📍` を同じ行に持つ行で 2 ブロックに割れる（本書「1 投稿から複数の出演情報」） |
...
| 日付行と `📍` 行が別の 2 公演告知 | 27.txt | 境界が取れず 1 ブロックになり、販売期間の日付まで候補に入って枠を対応付けられない | **将来対応**（本書「将来対応する投稿型」） |
...
27.txt  日付行と 📍 行が別の 2 公演   ブロックに割れず、販売期間の日付で枠を対応付けられない
```
...
公演日として採用した日付は、対応する 20 件すべてで告知の曜日と実際の曜日が一致した
...
| 対象 | 方法 |
| --- | --- |
| 抽出対象の判定 | 実サンプル 28 件が、対応 20 件 / 対象外 8 件に正しく仕分けられること（本書「抽出対象の判定」） |
...
- 抽出対象の 20 件が期待どおり抽出できること（本書「実サンプルでの検証結果」）
- 対象外の 8 件が 1 件も抽出されないこと（本書「実サンプルでの検証結果」）
